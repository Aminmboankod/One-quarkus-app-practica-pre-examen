...
add constraint orden_item_fk foreign key (ord_item) references t_items;
alter table t_ordenes
add constraint orden_user_fk foreign key (ord_user) references t_users;
insert into t_users (user_nom, user_prop)
values ('Doobey', 15),
    ('Hermione', 100);
insert into t_items (item_nom, item_prop, item_tipo)
values ('+5 Dexterity Vest', 20, 'NormalItem'),
    ('Elixir of the Mongoose', 7, 'NormalItem'),
    ('AgedBrie', 10, 'NormalItem');
insert into t_ordenes (ord_user, ord_item)
values ('Doobey','Elixir of the Mongoose'),
